...
static void pxt4_mark_recovery_complete(struct super_block *sb,
					struct pxt4_super_block *es);
static void pxt4_clear_journal_err(struct super_block *sb,
				   struct pxt4_super_block *es);
...
int pxt4_alloc_flex_bg_array(struct super_block *sb, pxt4_group_t ngroup)
{
	...
	if (!new_groups) {
		pxt4_msg(sb, KERN_ERR, "not enough memory for %d flex groups",
...
static int pxt4_fill_flex_info(struct super_block *sb)
{
	...
	for (i = 0; i < sbi->s_groups_count; i++) {
		...
		atomic_add(pxt4_free_inodes_count(sb, gdp),
			   &sbi->s_flex_groups[flex_group].free_inodes);
		atomic64_add(pxt4_free_group_clusters(sb, gdp),
			     &sbi->s_flex_groups[flex_group].free_clusters);
		atomic_add(pxt4_used_dirs_count(sb, gdp),
...
#ifndef CONFIG_QUOTA
	if (pxt4_has_feature_quota(sb) && !readonly) {
		pxt4_msg(sb, KERN_ERR,
			 "Filesystem with quota feature cannot be mounted RDWR "
			 "without CONFIG_QUOTA");
		return 0;
	}
	if (pxt4_has_feature_project(sb) && !readonly) {
		pxt4_msg(sb, KERN_ERR,
...
int pxt4_calculate_overhead(struct super_block *sb)
{
	...
	else if (pxt4_has_feature_journal(sb) && !sbi->s_journal) {
...
static void pxt4_clamp_want_extra_isize(struct super_block *sb)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
	struct pxt4_super_block *es = sbi->s_es;
	...
	if (PXT4_GOOD_OLD_INODE_SIZE + sbi->s_want_extra_isize >
							sbi->s_inode_size) {
		sbi->s_want_extra_isize = sizeof(struct pxt4_inode) -
						       PXT4_GOOD_OLD_INODE_SIZE;
		pxt4_msg(sb, KERN_INFO,
...
static int pxt4_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (le32_to_cpu(es->s_rev_level) == PXT4_GOOD_OLD_REV) {
		 ...
		 * struct pxt4_inode. Checking for that field should suffice to ensure
	...
	if (blocks_count > ((uint64_t)1<<32) - PXT4_DESC_PER_BLOCK(sb)) {
		pxt4_msg(sb, KERN_WARNING, "groups count too large: %u "
...
no_journal:
	...
	pxt4_clamp_want_extra_isize(sb);
				   ...
				   unsigned int journal_inum)
{
	...
	BUG_ON(!pxt4_has_feature_journal(sb));
			     ...
			     unsigned long journal_devnum)
{
	...
	if (journal_inum && journal_dev) {
		pxt4_msg(sb, KERN_ERR, "filesystem has both journal "
		       "and inode journals!");
		return -EINVAL;
	}

	if (journal_inum) {
		if (!(journal = pxt4_get_journal(sb, journal_inum)))
			return -EINVAL;
	} else {
		if (!(journal = pxt4_get_dev_journal(sb, journal_dev)))
			return -EINVAL;
	}
					...
					struct pxt4_super_block *es)
{
	...
	if (jbd3_journal_flush(journal) < 0)
		goto out;
...
static int pxt4_remount(struct super_block *sb, int *flags, char *data)
{
	...
	if ((sbi->s_mount_opt ^ old_opts.s_mount_opt) & PXT4_MOUNT_DAX) {
		pxt4_msg(sb, KERN_WARNING, "warning: refusing change of "
			"dax flag with busy inodes while remounting");
		sbi->s_mount_opt ^= PXT4_MOUNT_DAX;
	}
