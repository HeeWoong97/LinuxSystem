		    ...
		    unsigned int offset, struct pxt4_dir_entry_2 **res_dir)
{
	...
	while ((char *) de < dlimit) {
		...
		if ((char *) de + de->name_len <= dlimit &&
			 ...
			 * a full check */
			if (pxt4_check_dir_entry(dir, NULL, de, bh, bh->b_data,
						 bh->b_size, offset))
				return -1;
			*res_dir = de;
			return 1;
...
bool pxt4_empty_dir(struct inode *inode)
{
	unsigned int offset;
	struct buffer_head *bh;
	struct pxt4_dir_entry_2 *de, *de1;
	struct super_block *sb;
	...
	de = (struct pxt4_dir_entry_2 *) bh->b_data;
	de1 = pxt4_next_entry(de, sb->s_blocksize);
	if (le32_to_cpu(de->inode) != inode->i_ino ||
			le32_to_cpu(de1->inode) == 0 ||
			strcmp(".", de->name) || strcmp("..", de1->name)) {
		pxt4_warning_inode(inode, "directory missing '.' and/or '..'");
		brelse(bh);
		return true;
	}
	offset = pxt4_rec_len_from_disk(de->rec_len, sb->s_blocksize) +
		 pxt4_rec_len_from_disk(de1->rec_len, sb->s_blocksize);
	de = pxt4_next_entry(de1, sb->s_blocksize);
